...
static inline int inline_strcmp(const std::string& s1, const std::string& s2) {
    int counter = 0;
    // Compare lengths
    if (s1.size() != s2.size()) {
        //std::string msg = "Length mismatch: s1.size() = " + std::to_string(s1.size()) + ", s2.size() = " + std::to_string(s2.size());
        //MessageBoxA(nullptr, msg.c_str(), "Debug - Length Comparison", MB_OK);
        return false;
    }
    counter++;

    // If lengths are equal, compare characters
    std::size_t length = s1.size();
    for (std::size_t i = 0; i < length; ++i) {
        if (s1[i] != s2[i]) {
            //std::string msg = "Character mismatch at position " + std::to_string(i) + ": s1[i] = " + std::to_string(static_cast<unsigned char>(s1[i])) + ", s2[i] = " + std::to_string(static_cast<unsigned char>(s2[i]));
            //MessageBoxA(nullptr, msg.c_str(), "Debug - Character Comparison", MB_OK);
            if (std::to_string(static_cast<unsigned char>(s1[i])) != std::to_string(static_cast<unsigned char>(s2[i])))
                return false;
        }
    }
    counter++;
